...
public class ElementsTools{
    ...
    public static void centralizeElement(WebElement element){
        int center = (Driver.getInstance().manage().window().getSize().height + element.getSize().getHeight()) / 2;
    ...
    public static void clickElement(WebElement element, String elementName){
        try{
            element.click();
            Report.log(Status.INFO, "Clique realizado no botao '"+elementName+"'.", false);
        }catch(Exception error){
            Report.log(Status.FAIL, "Erro ao Clicar no botao '"+elementName+"'!" + System.lineSeparator() +
                                            "Erro encontrado = " + error, true);
            throw error;
        }
    }
        ...
        try{
            switch(selectBy){
                ...
                case SELECT_BY_VISIBLE_TEXT:
                    ...
                    break;
                default:
                    throw new SelectTypeNotImplementedException("Nao ha implementacao para esse tipo de Select "+selectBy.name()+"!");
            }
            Report.log(Status.INFO, "Foi selecionada a opcao '"+value+"' do campo '"+elementName+"'.", false);
            
        }catch(Exception error){
            Report.log(Status.FAIL, "Erro ao selecionar a opcao '"+value+"' do campo '"+elementName+"'!" + System.lineSeparator() +
                                            "Erro encontrado = " + error, true);
            throw error;
        }
    }
